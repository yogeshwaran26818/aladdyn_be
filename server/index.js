...
app.get('/api/auth', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`🔑 STOREFRONT TOKEN CREATED FOR SHOP: ${shop}`)
        } else {
    ...
    try {
      ...
      console.log('🔍 DEBUG: Script Tag URL:', scriptTagUrl)
...
// Serve static chatbot widget file
app.use('/public', express.static(path.join(__dirname, 'public')))

// Chatbot Widget Script Endpoint - Serves static file with dynamic config
app.get('/chatbot-widget.js', async (req, res) => {
  try {
    ...
    const apiBaseUrl = process.env.APP_URL || `${protocol}://${host}`
    
    // Redirect to static file with parameters
    res.redirect(`/public/chatbot-widget.js?shop=${encodeURIComponent(shop)}&api=${encodeURIComponent(apiBaseUrl)}`)
  } catch (error) {
